...
{
    private readonly ILogger _log = Log.ForContext("SourceContext", nameof(WslAvailableDistrosAdaptiveCardSession));
    ...
    private readonly List<WslDistributionInfo> _availableDistributionsToInstall;
    ...
    public WslAvailableDistrosAdaptiveCardSession(List<WslDistributionInfo> availableDistributions, IStringResource stringResource)
    {
    ...
    private ProviderOperationResult GetWslInstallationFormAdaptiveCard()
    {
        try
        {
            ...
            var noDescriptionFoundForWslDistribution = _stringResource.GetLocalized("NoDescriptionFoundForWslDistribution");
            ...
            {
                if (string.IsNullOrEmpty(distribution.Description))
                {
                    distribution.Description = _stringResource.GetLocalized("NoDescriptionFoundForWslDistribution");
                }
                ...
                var dataJson = new JsonObject
                {
                    ...
                    { "ContentDialogInfo", description },
                    { "Publisher", distribution.Publisher },
                    { "NoDistributionsFoundErrorVisibility", false },
            ...
            // Add error message for when there are no distributions available to install
            if (_availableDistributionsToInstall.Count == 0)
            {
                var noDistributionErrorData = new JsonObject
                {
                    { "NoDistributionsFoundError", noDistributionsFound },
                    { "NoDistributionsFoundErrorVisibility", true },
                };

                jsonArrayOfAvailableDistributions.Add(noDistributionErrorData);
            }
    ...
    private ProviderOperationResult GetWslReviewFormAdaptiveCardAsync(string inputJson)
    {
        try
        {
            ...
            var newDistroNameLabel = _stringResource.GetLocalized("DistributionNameLabel");
            var primaryButtonForCreationFlowText = _stringResource.GetLocalized("PrimaryButtonLabelForCreationFlow");
            var secondaryButtonForCreationFlowText = _stringResource.GetLocalized("SecondaryButtonLabelForCreationFlow");
            var providerDisplayName = _stringResource.GetLocalized("ProviderDisplayName");
            var base64Logo = string.IsNullOrEmpty(distribution.Base64StringLogo) ? _defaultWslLogo : distribution.Base64StringLogo;
            ...
            var reviewFormData = new JsonObject
            {
                ...
                { "DistributionNameLabel", newDistroNameLabel },
