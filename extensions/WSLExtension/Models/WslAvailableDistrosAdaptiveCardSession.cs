...
public class WslAvailableDistrosAdaptiveCardSession : IExtensionAdaptiveCardSession2
{
    ...
    private readonly List<WslDistributionInfo> _availableDistributions;
    ...
    public WslAvailableDistrosAdaptiveCardSession(List<WslDistributionInfo> distroList, IStringResource stringResource)
    {
    ...
    public string LoadTemplate(SessionState state)
    {
        ...
        {
            var storageFile = await StorageFile.GetFileFromApplicationUriAsync(new Uri($"ms-appx:///WslTemplates/{pathToTemplate}"));
