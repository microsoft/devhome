...
public class WslInstallAndRegisterDistroOperation : ICreateComputeSystemOperation
{
    ...
    public WslInstallAndRegisterDistroOperation(WslDistributionInfo distributionInfo, IStringResource stringResource, IWslManager wslManager)
    {
    ...
    public IAsyncOperation<CreateComputeSystemResult> StartAsync()
    {
        return Task.Run(async () =>
        {
            ...
            WslDistributionInfo? installedDistribution = null;
            var distributionInstalledSuccessfully = false;

            while (!cancellationTokenSource.IsCancellationRequested)
            {
                await Task.Delay(TimeSpan.FromSeconds(ThreeSecondDelayInSeconds));
                installedDistribution = await _wslManager.GetRegisteredDistributionAsync(_distributionInfo.Name);
