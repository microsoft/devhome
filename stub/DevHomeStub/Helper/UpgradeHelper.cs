...
#pragma warning restore CA2201 // Do not raise reserved exception types
        ...
        catch (Exception e)
        {
            ...
            if (unchecked((uint)e.HResult) == 0x80240438)
            {
                errorState = UpgradeState.NetworkError;
            }

            // WU_E_REDIRECTOR_CONNECT_POLICY - Error when updates are blocked by policy
            else if (unchecked((uint)e.HResult) == 0x8024500c)
            {
    ...
    private void SystemEvents_SessionEnding(object sender, Microsoft.Win32.SessionEndingEventArgs e)
    {
        this.Progress = 999;
        Thread.Sleep(300);
        _ = NativeMethods.RegisterApplicationRestart(_protocolHandlerArguments, 1 | 2 | 8 /* only restart on patch */);
    }
