...
/// <summary>
/// Used as the viewmodel for views that contain package information to install
/// a single package from the Microsoft store.
/// </summary>
public partial class StorePackageViewModel : ObservableObject
{
    private readonly ILogger _log = Log.ForContext("SourceContext", nameof(StorePackageViewModel));
        ...
        IMicrosoftStoreService microsoftStoreService)
    {
        ...
        {
            OnEventAction = (instance, source, args) => PackageInstallationChanged(instance, args),
    ...
    public void PackageInstallationChanged(object sender, AppInstallManagerItemEventArgs args)
    {
        ...
        _dispatcherQueue.TryEnqueue(() =>
        {
            ...
            switch (status.InstallState)
            {
                case AppInstallState.Error:
                    // For V1, installation updates happen in the store pdp dialog. However, in
                    // the future we can provide updates directly in Dev Home via this PackageInstallationChanged
                    // method.
                    _log.Error($"Failed to install {PackageFamilyName}. Error code: {status.ErrorCode:X}");
                    CurrentButtonContentKey = PackageInstalledResourceKey;
                    break;
                case AppInstallState.Canceled:
                    _log.Error($"Install for {PackageFamilyName} canceled");
                    CurrentButtonContentKey = GetButtonResourceKey;
                    break;
                case AppInstallState.Completed:
                    _log.Error($"Install for {PackageFamilyName} completed");
                    CurrentButtonContentKey = PackageInstalledResourceKey;
                    break;
                default:
