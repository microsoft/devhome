...
public class DataExtractor
{
    ...
    public static List<OperationsViewModel> FillDotButtonOperations(ComputeSystem computeSystem, WinUIEx.WindowEx windowEx)
    {
        ...
        {
            operations.Add(new OperationsViewModel("Restart", "\uE777", computeSystem.RestartAsync, windowEx));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.Delete))
        {
            operations.Add(new OperationsViewModel("Delete", "\uE74D", computeSystem.DeleteAsync, windowEx));
        }
    ...
    public static List<OperationsViewModel> FillLaunchButtonOperations(ComputeSystem computeSystem)
    {
        ...
        {
            operations.Add(new OperationsViewModel("Start", "\uE768", computeSystem.StartAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.ShutDown))
        {
            operations.Add(new OperationsViewModel("Stop", "\uE71A", computeSystem.TerminateAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.CreateSnapshot))
        {
            operations.Add(new OperationsViewModel("Checkpoint", "\uE7C1", computeSystem.CreateSnapshotAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.RevertSnapshot))
        {
            operations.Add(new OperationsViewModel("Revert", "\uE7A7", computeSystem.RevertSnapshotAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.Pause))
        {
            operations.Add(new OperationsViewModel("Pause", "\uE769", computeSystem.PauseAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.Resume))
        {
            operations.Add(new OperationsViewModel("Resume", "\uE768", computeSystem.ResumeAsync, null));
        }

        if (supportedOperations.HasFlag(ComputeSystemOperations.Terminate))
        {
            operations.Add(new OperationsViewModel("Terminate", "\uEE95", computeSystem.TerminateAsync, null));
        }
