...
public partial class WinLogsPageViewModel : ObservableObject, IDisposable
{
    ...
    public WinLogsPageViewModel()
    {
        // Log feature usage.
        _logMeasures = true;
        App.Log("DevHome_DevInsights_WinLogs_PageInitialize", LogLevel.Measure);
    ...
    public void LogStateChanged(object sender, RoutedEventArgs e)
    {
        ...
        if ((box is not null) && (box.Tag is not null))
        {
            ...
            if (_logMeasures)
            {
                App.Log("DevHome_DevInsights_WinLogs_LogStateChanged", LogLevel.Measure, new LogStateChangedEventData(box.Name, (box.IsChecked ?? false) ? "true" : "false"), null);
            }
    ...
    private void ClearWinLogs()
    {
        ...
        {
            // Log feature usage.
            App.Log("DevHome_DevInsights_WinLogs_ClearLogs", LogLevel.Measure);
        }
