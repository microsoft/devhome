...
public class WERAnalysisReport
{
    ...
    private void ParseRawAnalysis(string rawAnalysis)
    {
        ...
        try
        {
            ...
            {
                analysisText.AppendLine("Failing file: " + failureSourceFile.InnerText);
                if (!string.IsNullOrEmpty(failureSourceFileLineNumber?.InnerText))
                {
                    analysisText.AppendLine("Line Number: " + failureSourceFileLineNumber.InnerText);
                }
        ...
        {
            Analysis = "Unable to parse analysis results";
