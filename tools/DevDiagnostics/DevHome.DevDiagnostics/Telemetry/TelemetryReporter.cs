...
internal sealed class TelemetryReporter : IDisposable
{
    ...
    private void InteractiveUsageVisibilityEvent(object? sender, WindowEventGenerator.InteractiveUsageEventArgs e)
    {
        ...
        if (app != null)
        {
            ...
            {
                App.Log<VisibilityStartEventData>("DevHome_DevDiagnostics_VisibilityStart", LogLevel.Measure, new VisibilityStartEventData(featureName, DateTime.Now));
                Log.Debug("Visibility Start!");
            }
            else
            {
                App.Log<VisibilityStopEventData>("DevHome_DevDiagnostics_VisibilityStop", LogLevel.Measure, new VisibilityStopEventData(featureName, DateTime.Now));
    ...
    private void InteractiveUsageFocusEvent(object? sender, WindowEventGenerator.InteractiveUsageEventArgs e)
    {
        ...
        if (app != null)
        {
            ...
            if (e.UsageType == WindowEventGenerator.InteractiveUsageEventType.Start)
            {
                App.Log<VisibilityStartEventData>("DevHome_DevDiagnostics_FocusStart", LogLevel.Measure, new VisibilityStartEventData(featureName, DateTime.Now));
                Log.Debug("Focus Start!");
            }
            else
            {
                App.Log<VisibilityStopEventData>("DevHome_DevDiagnostics_FocusStop", LogLevel.Measure, new VisibilityStopEventData(featureName, DateTime.Now));
