...
public partial class ExpandedViewControlViewModel : ObservableObject
{
    ...
    private void AddPagesIfNecessary(Process? process)
    {
        ...
        // If App Details is missing, add all other _pages.
    ...
    {
        // First navigate to ProcessListPage, then remove all other _pages.
    ...
    public void NavigateTo(Type viewModelType)
    {
        for (var i = 0; i < Links.Count; i++)
        {
            ...
            if (link.PageViewModel == viewModelType)
            {
                ...
                {
                    // Navigation between _pages is a bit convoluted, as we set SelectedNavLinkIndex, which triggers a change
                    ...
                    // is a ListView Item, but there are several _pages under Settings). If we end up navigating to the same page,
