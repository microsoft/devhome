...
public class ExtensionJsonTests
{
    ...
    {
        string absoluteSchemaPath = Path.Combine(Directory.GetCurrentDirectory(), InternalExtensionJsonSchemaRelativeFilePath);
        Assert.IsTrue(File.Exists(absoluteSchemaPath), $"The schema file '{absoluteSchemaPath}' does not exist.");

        string absoluteExtensionJsonPath = Path.Combine(Directory.GetCurrentDirectory(), InternalExtensionJsonRelativeFilePath);
